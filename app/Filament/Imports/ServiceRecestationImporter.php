...
class ServiceRecestationImporter extends Importer
{
    ...
    public function resolveRecord(): ServiceRecestation
    {
        $record = ServiceRecestation::firstOrNew([
            'name' => $this->data['name'],
        ]);
        ...
        if (! $record->exists) {
            ...
            // default — every new row must set them explicitly.
    ...
    {
        $body = 'Your service recestation import has completed and '.Number::format($import->successful_rows).' '.str('row')->plural($import->successful_rows).' imported.';
