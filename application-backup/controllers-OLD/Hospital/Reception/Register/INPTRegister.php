...
class INPTRegister extends MY_Controller
{
    ...
    public function index(){
        //  && $this->aauth->is_allowed('Hospital INPT Register','Hospital INPT')
        if($this->isLoggedIn()) {

            $this->_pageData['title'] = 'Hospital INPT Register';
            $this->_pageData['module'] = 'Hospital INPT Register';

            $html = $this->load->makeViewWithOutTemplate('inpt_register', $this->_pageData, true);
            $this->makeView($html);
        }else{
        ...
        if($this->isLoggedIn()) {
            ...
            $columns = array(
                array('db' => 'id', 'table' => $table, 'dt' => 0,'as' => 'id','formatter' => function ($d, $row) {
                    ...
                    $html = '<a href="' . (site_url($this->_pageData['urlsToRemember']['HOSPITAL_REC_PATH_INP_PATIENT_PROFILE'] . $row['id'])) . '" target="_blank" >'.'INPT_MR_'.str_pad($d, 8, '0', STR_PAD_LEFT).'</a>'.'<br>';
                    $html .= '<a title="Site Traking Number: '.str_pad($row['patient_id'], 8, '0', STR_PAD_LEFT).'">'.'OPD_'.str_pad($d, 8, '0', STR_PAD_LEFT).'</a>';
               ...
               // array('db' => 'service_name', 'table' => $table,'as' => 'service_name', 'dt' => 4),
                        ...
                        if($row['service_name'] == NULL){
                          ...
                          $html = '<a href="' . (site_url($this->_pageData['urlsToRemember']['OPD_NEW_TREATMENTS'] . $row['id'])) . '" target="_blank" class="btn btn-sm btn-primary">Select Service</a>';
                           return $html;
                            ...
                            $html .= '<a href="' . (site_url($this->_pageData['urlsToRemember']['HOSPITAL_REC_PATH_INP_PATIENT_PROFILE'] . $row['id'])) . '" class="btn btn-sm btn-default" title="Inpatient File for '. $row['pateint_name'] .'" ><i class="fas fa-id-badge"></i></a>';
                ...
                array('db' => 'inpatient_patient_id', 'table' => $table, 'dt' => 9,'as' => 'inpatient_patient_id'),
            );
