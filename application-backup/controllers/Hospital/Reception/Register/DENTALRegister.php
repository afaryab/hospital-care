...
class DENTALRegister extends MY_Controller
{
    ...
    public function index(){
        ...
        if($this->isLoggedIn()) {
            ...
            $this->_pageData['title'] = 'Hospital Dental Register';
            $this->_pageData['module'] = 'Hospital Dental Register';

            $html = $this->load->makeViewWithOutTemplate('dental_register', $this->_pageData, true);
            $this->makeView($html);
        }else{
        ...
        if($this->isLoggedIn()) {
            ...
            $columns = array(
                array('db' => 'id', 'table' => $table, 'dt' => 0,'as' => 'id','formatter' => function ($d, $row) {
                    return 'DENTAL_TRT_'.str_pad($d, 8, '0', STR_PAD_LEFT).'<br/>'.'<a title="Site Traking Number: '.str_pad($row['patient_id'], 8, '0', STR_PAD_LEFT).'">'.'DENTAL_'.str_pad($d, 8, '0', STR_PAD_LEFT).'</a>';
            ...
            echo json_encode(
                SSP::simplewithjoin($_GET, $sql_details, $table, $primaryKey, $columns,'patients','dental_treatments`.`patient_id` = `patients`.`id')
            );
        }
    }
