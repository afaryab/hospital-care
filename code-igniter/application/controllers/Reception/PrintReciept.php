...
class PrintReciept extends MY_Controller
{
        ...
        if($this->isLoggedIn()) {
            ...
            if($receiptId == 0){
                ...
                if(array_key_exists('receipt_id',$_GET)){
                    ...
                    redirect($this->_pageData['PRINT_RECEIPT'].$_GET['receipt_id']);
                }
            }
            ...
            $html = $this->load->makeViewWithOutTemplate('print_reciept', $this->_pageData, true);
            $this->makeView($html);
        }else{
