<link href="<?php echo base_url('public/fonts/stylesheet.css') ?>" rel="stylesheet" type="text/css"/>
<style>
@media print{
    table>tbody>tr>td{
        border-top: none !important;
        border-bottom: none !important;
        line-height: 50px;
        padding: 8px;
    }
    .logo{
        max-width: 140px !important;
        margin-top: 0px !important;
        padding-top: 0px !important;
    }
    hr{
        border-top: 1px solid #A6A5A5 !important;
    }
    .pdl{
        padding-left: 50px;
    }
    .portlet.light {
        padding: 0px !important;
    }
    table tr td:first-child{
        width:150px;
    }
    table tr td:nth-child(2){
        font-weight: 700;
        font-size:16px;
    }
    table tr td:nth-child(4){
        font-weight: 700;
        font-size:16px;
    }
}
.pdl{
        padding-left: 50px !important;
    }
    table tr td:first-child{
        width:150px;
    }
    table tr td:nth-child(2){
        font-weight: 700;
        font-size:16px;
    }
    table tr td:nth-child(4){
        font-weight: 700;
        font-size:16px;
    }
</style>
 
<!-- END SIDEBAR -->
<!-- BEGIN CONTENT -->
<?php 
$count = 0; ?>
<div class="page-content-wrapper">
    <div class="page-content">
        <!-- BEGIN PAGE CONTENT-->
        <div class="row">
            <div class="col-md-12 ">
                <!-- BEGIN SAMPLE FORM PORTLET-->
                <div class="portlet light bordered">
                    <div class="col-lg-12 text-center invoice-header">
                        <img class="img-responsive logo" style="display: inline-block; margin: 10px 0;max-width: 200px;" src="<?= $PRINT_IMAGE_64; ?>">
                    </div>
                    <div class="portlet-body form">
                    
                        <div class="row" >
                            <div class="col-md-12">
                                <table id="example" class=" display" cellspacing="50" width="100%">
                                    <?php $sp='&nbsp';
                                    $gender='Male';
                                    if(isset($patient['gender'])){
		                            if($patient['gender']=='f' || $patient['gender']=='F'){
		                            	$gender='Female';
		                            }
		                            if($patient['gender']=='m' || $patient['gender']=='M'){
		                            	$gender='Male';
		                            }
                                    }
                                    ;?>
                                    <tr>
                                        <td><b>Room Number :</b></td>
                                        <td><?= isset($files['room_name']) ? $files['room_name'] : $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="text-center"><b>Mr No. :</b></td>
                                        <td><?= str_pad($files['id'], 8, '0', STR_PAD_LEFT) ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr>
                                    
                                    <tr>
                                        <td><b>Patient Name :</b></td>
                                        <td><?= isset($patient['pateint_name']) ? $patient['pateint_name'] : $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="text-center"><b>Age / Sex</b></td>
                                        <td><?= isset($patient['age_days']) ? $patient['age_days'] : $sp; ?> / <?= $gender; ?> <hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr>
                                    <tr>
                                        <td ><b>Husband / Father Name :</b></td>
                                        <td ><?= isset($patient['guardian']) ? $patient['guardian'] : $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="text-center"><b>Address:</b></td>
                                        <td ><?= isset($patient['patient_address']) ? $patient['patient_address'] : $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr>
                                    
                                    <tr>
                                        <td><b>CNIC :</b></td>
                                        <td><?= isset($patient['patient_cnic']) ? $patient['patient_cnic'] : $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="text-center"><b>Cell No.</b></td>
                                        <td><?= isset($patient['patient_contact_mobile']) ? $patient['patient_contact_mobile'] : $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr>
                                    <tr>
                                        <td ><b>Provisional Diagnosis:</b></td>
                                        <td ><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="text-center"><b>Final Diagnosis:</b></td>
                                        <td ><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr>
                                    
                                    <tr>
                                        <td><b>Operation / Procedure</b></td>
                                        <td ><?= isset($files['service_name']) ? $files['service_name'] : $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="text-center"><b>Consultant</b></td>
                                        <td ><?= isset($this->aauth->get_user($files['treatment_by'])->name) ? $this->aauth->get_user($files['treatment_by'])->name : $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    
                                    <tr>
                                        
                                    </tr>
                                    <tr>
                                        <td><b>Date of Procedure :</b></td>
                                        <td><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="text-center"><b>Cash / Panel</b></td>
                                        <?php if($files['panel_name'] != NULL){ ?>
                                            <td>Panel ( <?= $files['panel_name']?> )<hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        </br>
                                        <?php }else{ ?>  
                                            <td>Cash<hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <?php } ?>
                                        <td><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr>
                                    <tr>
                                        <td><b>Date of Admission :</b></td>
                                        <td><?= isset($files['created_on']) ? $files['created_on'] : $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="text-center"><b>Date of Discharge</b></td>
                                        <td><?php if($files['closed_on'] != NULL ){ echo date('d-m-Y', strtotime($files['closed_on'])); }else{ echo $sp; } ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr>
                                    <tr>

                                        
                                        
                                        <td></td>
                                        <td class="" style="font-size:20px;"><b>Blood Report</b></td>
                                        <td></td>
                                        <td class="" style="font-size:20px;"><b>Consultant Visit & Other Charges</b></td>
                                        
                                        
                                    </tr>
                                    <tr>
                                        <td ><b>HB/CBC</b></td>
                                        <td  ><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="pdl" colspan="6"><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr>
                                    <tr>
                                        <td ><b>BSR</b></td>
                                        <td ><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="pdl" colspan="6"><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>

                                    </tr>
                                    <tr>
                                        <td ><b>LFT</b></td>
                                        <td ><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="pdl" colspan="6"><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr>
                                    
                                    <tr>
                                        <td ><b>HBs Ag</b></td> 
                                        <td><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="text-center"><b>BSR</b></td>
                                        <td ><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr>
                                    <tr>
                                        <td ><b>RFT</b></td>
                                        <td><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="text-center"><b>Oxygen</b></td>
                                        <td ><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr>
                                    <tr>
                                        <td ><b>Urine C/E</b></td>
                                        <td><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="text-center"><b>Phototherapy</b></td>
                                        <td ><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr>
                                    <tr>
                                        <td ><b>Anti HCV</b></td>
                                        <td><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="text-center"><b>Room</b></td>
                                        <td ><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr><tr>
                                        <td ><b>HIV</b></td>
                                        <td><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                        <td class="text-center"><b>Other Charges</b></td>
                                        <td ><?= $sp; ?><hr style="border-top: 2px solid #A6A5A5;"/></td>
                                    </tr>
                                    
                                </table>
                            </div>
               
                            
                            
                        </div>
                        

                    
                    </div>
                </div>
            </div> 
        </div>
    </div>
</div>
<style>
    .dz-details{
        display: none !important;
    }
    .table-nopadding>tbody>tr, .table-nopadding>tbody>tr>td{
        padding: 0px !important;
    }
</style>

