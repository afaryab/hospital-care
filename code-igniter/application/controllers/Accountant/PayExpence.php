...
class PayExpence extends MY_Controller
{
...
//&& $this->aauth->is_allowed('Add New Expense Voucher','Accounts')
        if($this->isLoggedIn() ) {
            ...
            if($this->havePost()){
                ...
                try {
                    ...
                    if($_POST['payment_reference'] === 0 || $_POST['payment_reference'] === ''){
                        $errors[] = 'Please do provide some detail for Accounts.';
                    }
                            ...
                            try {
                                ...
                                if(empty($errors) && !empty($voucher)) {
                                    ...
                                    $newcounter = [
                                        ...
                                        'status' => 'CLOSED',
                                        'opening_amount' => 0,
                                        'closing_amount_cash' => 0,
                                        'closing_amount' => 0,
                                        'expense_payed' => $_POST['amount_received_num'],
                                    ];
                                    ...
                                    $this->setMessage('success', 'Expense Payment #' . $voucherIDexp . ' is payed!');
                                    $this->activityLog('Expense Payment #' . $voucherIDexp . ' is payed!');
            ...
            $this->_pageData['title'] = 'Pay Expence';
            $this->_pageData['module'] = 'Pay Expence';
            $this->_pageData['users'] = $this->aauth->list_users();
            $html = $this->load->makeViewWithOutTemplate('pay_expence', $this->_pageData, true);
            $this->makeView($html);
        }else{
    ...
    public function PrintVoucher($expenseId = 0){
        if($this->isLoggedIn()) {
            ...
            $this->_pageData['module'] = 'ExpenseVouchers';
