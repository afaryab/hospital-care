...
class DentalPatientProfile extends MY_Controller
{
    ...
    public function index($id = 0){
        ...
        if($this->isLoggedIn()) {
                if($id != 0 ){
                    ...
                    $this->_pageData['files'] = $this->dental_patient_file->findOneBy(['id' => $id]);
                    ...
                    $this->_pageData['trans'] = $this->dental_transactions->findBy(['file_id' => $id]);

                    $this->load->model('commonModel','dental_appointments');
                    $this->dental_appointments->setTableName('dental_appointments');
                    $this->_pageData['appointments'] = $this->dental_appointments->findBy(['file_id' => $id]);
                    ...
                    $this->_pageData['treats'] = $this->dental_treatments->findBy(['file_id' => $id]);
...
// echo "The time in " . date_default_timezone_get() . " is " . date("H:i:s");
                    ...
                    if($this->havePost())
                    {
                        ...
                        $this->dental_patient_file->updateRecord($id,$fileupdate);
                        
                        redirect($this->_pageData['HOSPITAL_DNTL_REGISTER']);
                    }
                

                    $this->_pageData['module'] = 'Hospital';
                    $html = $this->load->makeViewWithOutTemplate('dental_profile', $this->_pageData, true);
                    $this->makeView($html);
                }else{
