...
class DentalMRBook extends MY_Controller
{
    ...
    public function index(){
        ...
        if($this->isLoggedIn()) {
            ...
            $this->_pageData['title'] = 'Hospital Dental MRBook';
            $this->_pageData['module'] = 'Hospital Dental MRBook';

            $html = $this->load->makeViewWithOutTemplate('dental_mrbook', $this->_pageData, true);
            $this->makeView($html);
        }else{
        ...
        if($this->isLoggedIn()) {
            ...
            $columns = array(
                array('db' => 'id', 'table' => $table, 'dt' => 0,'as' => 'id','formatter' => function ($d, $row) {
                    ...
                    $html = '<a href="' . (site_url($this->_pageData['urlsToRemember']['HOSPITAL_REC_PATH_DENTAL_PATIENT_PROFILE'] . $row['id'])) . '" target="_blank" >'.'INPT_MR_'.str_pad($d, 8, '0', STR_PAD_LEFT).'</a>'.'<br>';
               ...
               // array('db' => 'service_name', 'table' => $table,'as' => 'service_name', 'dt' => 4),
                        ...
                        if($row['service_name'] == NULL){
                          ...
                          $html = '<a href="' . (site_url($this->_pageData['urlsToRemember']['OPD_NEW_TREATMENTS'] . $row['id'])) . '" target="_blank" class="btn btn-sm btn-primary">Select Service</a>';
                           return $html;
                            ...
                            $html .= '<a href="' . (site_url($this->_pageData['urlsToRemember']['HOSPITAL_REC_PATH_DENTAL_PATIENT_PROFILE'] . $row['id'])) . '" class="btn btn-sm btn-default" title="Inpatient File for '. $row['pateint_name'] .'" ><i class="fas fa-id-badge"></i></a>';
                ...
                array('db' => 'dental_patient_id', 'table' => $table, 'dt' => 9,'as' => 'dental_patient_id'),
            );
