...
class Aauth {
     ...
     */
    public function remind_password($email,$licenceKey = ''){
        ...
        if ($query->num_rows() > 0){
            ...
            $this->CI->email->data($data);
            $this->CI->email->set_protocol('bsofts');
     ...
     */
    public function create_user($email, $pass, $name='',$parentId = 0) {
        ...
        if ( $this->aauth_db->insert($this->config_vars['users'], $data )){
            ...
            $data['id'] = $this->aauth_db->insert_id();
            $queryString = $this->aauth_db->insert_string($this->config_vars['users'], $data);
    ...
    public function create_user_2($email, $pass, $name='') {
    ...
    public function update_user($user_id, $email = FALSE, $pass = FALSE, $name = FALSE) {
        ...
        if ($email != FALSE) {
            if ($this->user_exist_by_email($email)) {
                $EmailUser = $this->get_user_id($email);
                if($user_id != $EmailUser){
    ...
    public function update_user_type($user_id, $data) {
    ...
    public function update_user_custom($user_id, $custom = []) {
    ...
    public function update_group_custom($grp_id, $custom = []) {

        $this->aauth_db->where('id', $grp_id);
        return  $this->aauth_db->update($this->config_vars['groups'], $custom);


    }

    public function update_perm_custom($perm_id, $custom = []) {

        $this->aauth_db->where('id', $perm_id);
        return  $this->aauth_db->update($this->config_vars['perms'], $custom);
    ...
    public function list_users($group_par = FALSE, $limit = FALSE, $offset = FALSE, $include_banneds = FALSE) {
        ...
        if ($group_par != FALSE) {
            ...
            $this->aauth_db->select('*, '.$this->config_vars['users'].'.id as id')
    ...
    public function getOpdDoctors($valu)
    {
        $this->aauth_db->select('*')
        ->from($this->config_vars['users'])
        ->where($valu, 1);
     ...
     * @return array Array of users
     */
    public function list_users_with_variables($group_par = FALSE, $limit = FALSE, $offset = FALSE, $include_banneds = FALSE) {
        ...
        foreach($query->result() as $result){
            $returnArray[$result->id] = $result;
            $secQuery = $this->aauth_db->select('*')->from($this->config_vars['user_variables'])->where('user_id',$result->id)->get();
            if($secQuery->num_rows() > 0){
                foreach($secQuery->result_array() as $custom_vars){
                    $returnArray[$result->id]->custom_vars = [
                        $custom_vars['data_key'] => $custom_vars['value'],
                    ];
                }
     ...
     */
    public function ban_user($user_id,$message) {
        
        $data = array(
            'banned' => 1,
            'banned_message' => $message,
            'verification_code' => ''
        );
    ...
    public function get_user_groups($user_id = FALSE){
        ...
        $this->aauth_db->join($this->config_vars['groups'], $this->config_vars['groups'].".id =".$this->config_vars['user_to_group'].".group_id");
        $this->aauth_db->where($this->config_vars['user_to_group'].'.user_id', $user_id);

        $query = $this->aauth_db->get()->result();
     ...
     */
    public function create_group($group_name, $definition = '', $department = '', $url = 'Dashboard/ViewDashboard') {
    ...
    public function add_member($user_id, $group_par) {
        ...
        $group_id = $this->get_group_id(trim($group_par));
        ...
        $queryOFMember = $this->aauth_db->where('user_id',$user_id)->where('group_id',$group_id)->from($this->config_vars['user_to_group'])->get();

        if ($queryOFMember->num_rows() == 0) {
            $data = array(
                'user_id' => $user_id,
                'group_id' => $group_id
            );
            $this->aauth_db->trans_start();
            $return = $this->aauth_db->insert($this->config_vars['user_to_group'], $data);
            $data['id'] = $this->aauth_db->insert_id();
            $this->aauth_db->trans_complete();
            ...
            $ueryResult = $queryOFMember->row();
            $return = $this->aauth_db->where($ueryResult->id)->update($this->config_vars['user_to_group'], $data);
            $data['id'] = $ueryResult->id;
     ...
     * @return bool Add success/failure
     */
    public function update_user_key($userId,$key,$value){
        
        $this->aauth_db->where(['id' => $userId])->update($this->config_vars['users'], [$key => $value]);
    ...
    public function is_admin( $user_id = FALSE ) {
        ...
        if(is_object($user) && $user->is_super_admin == 1){
            return true;
        }else {
    ...
    public function get_perm($group_id) {
     ...
     */
    public function create_perm($perm_name, $perm_group, $definition='') {
     ...
     * @return bool
     */
    public function is_allowed($perm_par, $perm_group, $user_id=FALSE){
        ...
        if($perm_id == FALSE){
            ...
            $perm_id = $this->create_perm($perm_par,$perm_group);
    ...
    public function get_perms(){
        $query = $this->aauth_db->select('id,name,definition,perm_group')->from($this->config_vars['perms'])->get();
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($result as $row){
                $perms[$row->perm_group][$row->id] = $row;
            }
            return $perms;
        }
     ...
     * Get groups
     * GET all groups
     * @param string $group_name New group name | id
     * @return array
     */
    public function get_groups($group = '') {
        
        $query = $this->aauth_db->from($this->config_vars['groups'])->get();
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($result as $row){
                if($group != ''){
                    if($group == $row->id || $row->name == $group){
                        $groups = $row;
                    }
                }else{
                    $groups[$row->id] = $row;
