...
class DentalCounter extends MY_Controller
{
    ...
    public function index(){
        ...
        if($this->isLoggedIn()) {
            ...
            if($this->havePost() && array_key_exists('cart_services',$_POST)){
                ...
                if(array_key_exists('patient_id', $_POST) && $_POST['patient_id'] != 0){
                    ...
                    if(!$dentalPatient){
                        ...
                        $createdDentalPatientsThisMonth = $this->commonModel->countBy("DATE_FORMAT(created_on, '%Y-%m') = " . $ym_escaped);

                        $dentalPatientId = $this->commonModel->addNew([
                            'site_patient_id' => $patientId
                        ]);

                        $dentalPatient =  $this->commonModel->findOneBy(['id' => $dentalPatientId]);
                        $dentalPatientPSNumber = date("Y/m").'/DTL/'.str_pad($createdDentalPatientsThisMonth++, 6, '0', STR_PAD_LEFT);
                        ...
                        $this->dental_ps_numbers->addNew([
                            ...
                            'dental_patient_id' => $dentalPatientId,
                            'ps_number' => $dentalPatientPSNumber
                        ]);
                    }
                    ...
                    $createdDentalPatientsThisMonth = $this->dental_patients->countBy("DATE_FORMAT(created_on, '%Y-%m') = " . $ym_escaped);

                    $dentalPatientId = $this->dental_patients->addNew([
                        'site_patient_id' => $patientId
                    ]);
                    $dentalPatient =  $this->dental_patients->findOneBy(['id' => $dentalPatientId]);
                    $dentalPSNumber = date("Y/m").'/DTL/'.str_pad($createdDentalPatientsThisMonth++, 6, '0', STR_PAD_LEFT);
                ...
                foreach($_POST['cart_services'] as $cartService){
                    ...
                    if($cartService['servicetype'] == 'DENTAL'){
            ...
            $html = $this->load->makeViewWithOutTemplate('counter/dental_counter', $this->_pageData, true);
