...
class PATHRegister extends MY_Controller
{
    ...
    public function index(){
        // && $this->aauth->is_allowed('Hospital Pathology Register','Hospital Pathology')
        if($this->isLoggedIn()) {

            $this->_pageData['title'] = 'Hospital Pathology Register';
            $this->_pageData['module'] = 'Hospital Tests Register';

            $html = $this->load->makeViewWithOutTemplate('path_register', $this->_pageData, true);
            $this->makeView($html);
        }else{
        ...
        if($this->isLoggedIn()) {
            ...
            $columns = array(
                array('db' => 'id', 'table' => $table, 'dt' => 0,'as' => 'id','formatter' => function ($d, $row) {
                    return 'PATH_TST_'.str_pad($d, 8, '0', STR_PAD_LEFT).'<br/>'.'<a title="Site Traking Number: '.str_pad($row['patient_id'], 8, '0', STR_PAD_LEFT).'">'.'OPD_'.str_pad($d, 8, '0', STR_PAD_LEFT).'</a>';
