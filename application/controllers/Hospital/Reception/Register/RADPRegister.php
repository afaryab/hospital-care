...
class RADPRegister extends MY_Controller
{
    ...
    public function index(){
        //  && $this->aauth->is_allowed('Hospital RADP Register','Hospital RADP')
        if($this->isLoggedIn()) {

            $this->_pageData['title'] = 'Hospital RADP Register';
            $this->_pageData['module'] = 'Hospital Xray Register';

            $html = $this->load->makeViewWithOutTemplate('radp_register', $this->_pageData, true);
            $this->makeView($html);
        }else{
        ...
        if($this->isLoggedIn()) {
            ...
            $columns = array(
                array('db' => 'id', 'table' => $table, 'dt' => 0,'as' => 'id','formatter' => function ($d, $row) {
                    return 'RAD_'.str_pad($d, 8, '0', STR_PAD_LEFT).'<br/>'.'<a title="Site Traking Number: '.str_pad($row['patient_id'], 8, '0', STR_PAD_LEFT).'">'.'OPD_'.str_pad($d, 8, '0', STR_PAD_LEFT).'</a>';
                ...
                array('db' => 'treatment_by', 'table' => $table, 'dt' => 1,'as' => 'treatment_by','formatter' => function ($d, $row) {
                    ...
                    if($d == 0 || $d == NULL){
                        ...
                        $html = '<a href="' . (site_url($this->_pageData['urlsToRemember']['EMER_NEW_TREATMENTS'] . $row['id'])) . '" target="_blank" class="btn btn-sm btn-primary">Assign Doctor</a>';
                        return $html;
                    }else{
               ...
               // array('db' => 'service_name', 'table' => $table,'as' => 'service_name', 'dt' => 4),
                ...
                array('db' => 'opd_patient_id', 'table' => $table, 'dt' => 9,'as' => 'opd_patient_id'),
            );
            ...
            echo json_encode(
                SSP::simplewithjoin($_GET, $sql_details, $table, $primaryKey, $columns,'patients','opd_treatments`.`patient_id` = `patients`.`id')
            );
        }
    }
