...
class OPDRegister extends MY_Controller
{
    ...
    public function index(){
        ...
        if($this->isLoggedIn()) {
            ...
            $this->_pageData['title'] = 'Hospital OPD Register';
            $this->_pageData['module'] = 'Hospital OPD Register';

            $html = $this->load->makeViewWithOutTemplate('opd_register', $this->_pageData, true);
            $this->makeView($html);
        }else{
        ...
        if($this->isLoggedIn()) {
            ...
            $columns = array(
                array('db' => 'id', 'table' => $table, 'dt' => 0,'as' => 'id','formatter' => function ($d, $row) {
                    return 'OPD_TRT_'.str_pad($d, 8, '0', STR_PAD_LEFT).'<br/>'.'<a title="Site Traking Number: '.str_pad($row['patient_id'], 8, '0', STR_PAD_LEFT).'">'.'OPD_'.str_pad($d, 8, '0', STR_PAD_LEFT).'</a>';
