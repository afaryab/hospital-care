    ...
    public function JSON(){
        if($this->isLoggedIn()) {
            ...
            $columns = array(
                ...
                array('db' => 'amount_submitted', 'table' => $table,'as' => 'amount_submitted', 'dt' => 4),
                array('db' => 'amount_recieved', 'table' => $table ,'as' => 'amount_recieved', 'dt' => 5),
                array('db' => 'payment_reference', 'table' => $table,'as' => 'payment_reference', 'dt' => 6),
                array('db' => 'status', 'table' => $table,'as' => 'status', 'dt' => 7),
                array(
                    'db' => 'is_bill_submitted',
                    'as' => 'is_bill_submitted',
                    'dt' => 8,
                    'table' => $table,
                    'as' => 'is_bill_submitted',
                    'formatter' => function ($id, $row) {
                        ...
                        if($row['is_bill_submitted'] == 1){
                            $html = '<a href="' . (site_url($this->_pageData['urlsToRemember']['INP_PANEL_PAY'] . $row['mr_no'])) . '" target="_blank" class="btn btn-sm btn-primary">PAYMENT</a>
                            <a href="' . (site_url($this->_pageData['urlsToRemember']['SUBMIT_BILL'] . $row['mr_no'])) . '" target="_blank" class="btn btn-sm btn-primary">EDIT SUBMISSION</a>';
                        }else{
                            $html = '<a href="' . (site_url($this->_pageData['urlsToRemember']['INP_PANEL_PAY'] . $row['mr_no'])) . '" target="_blank" class="btn btn-sm btn-primary">PAYMENT</a>
                            <a href="' . (site_url($this->_pageData['urlsToRemember']['SUBMIT_BILL'] . $row['mr_no'])) . '" target="_blank" class="btn btn-sm btn-primary"> SUBMIT BILL</a>';
                        }
                        return $html;
