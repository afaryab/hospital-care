...
class BankIncomeStatement extends MY_Controller
{
    ...
    public function index(){
        if($this->isLoggedIn()) {
            ...
            $this->_pageData['title'] = 'Bank Income Statement';
            $this->_pageData['module'] = 'Bank Income Statement';
            ...
            if($dateType == 'S'){
                ...
                $transactions = $this->transaction->findBy(['type'=>'CARD','CAST(created_on AS DATE) = ' => $date ]);
                ...
                $transactions = $this->transaction->findBy(['type'=>'CARD','CAST(created_on AS DATE) >= ' => $date['start'], 'CAST(created_on AS DATE) <= ' => $date['end'] ]);
            ...
            $html = $this->load->makeViewWithOutTemplate('bank_income_statement', $this->_pageData, true);
            $this->makeView($html);
        }else{
