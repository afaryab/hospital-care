...
class CounterReceivingSummary extends MY_Controller
{
    ...
    public function index(){
        if($this->isLoggedIn()) {
            ...
            $this->_pageData['title'] = 'Counter Receiving Summary';
            $this->_pageData['module'] = 'Counter Receiving Summary';
            $this->_pageData['receptionactivity'] = [];
            ...
            if($dateType == 'S'){
                ...
                $this->_pageData['receptionactivity'] = $this->activity->findBy(['CAST(created_on AS DATE) >= ' => $date['start'], 'CAST(created_on AS DATE) <= ' => $date['end'] ]);
            ...
            $html = $this->load->makeViewWithOutTemplate('counter_receiving_summary', $this->_pageData, true);
            $this->makeView($html);
        }else{
