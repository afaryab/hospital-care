...
class InpatientPanelPayment extends MY_Controller
{
    ...
    public function index($id = 0){
        ...
        if($this->isLoggedIn()) {
            ...
            if($this->havePost()){
                ...
                redirect($this->_pageData['PANEL_PAYMENTS']);

            }
            
            
            $html = $this->load->makeViewWithOutTemplate('inp_panel_payments', $this->_pageData, true);
            $this->makeView($html);
    ...
    public function submit_bill($id = 0){

        if($this->isLoggedIn()) {

            $this->load->model('commonModel', 'commonModel');

            $this->_pageData['title'] = 'Panel Bill Submission';
            $this->_pageData['module'] = 'Panel Bill Submission';

            $this->load->model('commonModel','panel_payments');
            $this->panel_payments->setTableName('panel_payments');
            $this->_pageData['amount_info'] = $this->panel_payments->findOneBy(['mr_no' => $id]);

            if($this->havePost()){

                $this->load->model('commonModel','panel_payments');
                $this->panel_payments->setTableName('panel_payments');
                $panelID = $this->panel_payments->findOneBy(['mr_no' => $id]);

                $panelarray = [
                    'amount_submitted' => $_POST['amount_received_num'],
                    'is_bill_submitted' => 1,
                    'modified_on' => date('Y-m-d H:i:s')
                ];
            ...
            $html = $this->load->makeViewWithOutTemplate('inp_panel_bill_sumbission', $this->_pageData, true);
            $this->makeView($html);
