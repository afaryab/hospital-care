...
- `md-expert` — Medical domain expert for Punjab Healthcare Commission (PHC) compliance, patient safety, and AI governance in healthcare systems. Validates OPD/IPD/emergency workflows, ensures secure handling of patient data (RBAC, audit logs, masking), and enforces PHC MSDS standards. Reviews healthcare APIs, multi-tenant hospital SaaS, and incident reporting systems. Restricts unsafe AI usage (no autonomous diagnosis/prescription). Generates compliance reports, risk flags, and remediation steps. Use when building or reviewing hospital/clinic software, EMR/EHR, lab/pharmacy modules, or AI-assisted medical features. Triggers on "healthcare system", "hospital software", "patient records", "medical logs", "AI diagnosis", "PHC compliance", "incident reporting", or "medical workflow".
- `ui-implementation` — Frontend UI/UX implementation skill focused on Tailwind CSS, design consistency, responsive layouts, and production-quality interfaces. Use when building or refining Laravel Blade, Livewire, Inertia.js, React, or Filament screens. Creates polished dashboards, admin panels, forms, tables, cards, modals, tabs, onboarding flows, status indicators, and mobile-friendly layouts. Applies spacing systems, typography hierarchy, reusable component structure, empty states, validation feedback, loading states, accessibility basics, and conversion-friendly UI patterns. Improves weak layouts, removes clutter, and upgrades rough screens into modern usable interfaces. Triggers on "build UI", "implement frontend", "improve design", "Tailwind", "dashboard UI", "form UI", "admin panel", "responsive page", or "better UX".
