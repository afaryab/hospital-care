...
function ServiceOrderDetailPanel({
    ...
    const incomeElements = useMemo(
        () =>
            (so.transaction_elements ?? []).filter(
                (e) =>
                    e.income_or_expense === 'INCOME' && e.type !== 'RECES-IND',
            ),
        ...
        <>
            ...
            <div className="space-y-2 border-b pb-3">
                ...
                <Select
                    ...
                    onValueChange={(v) => setNewStatus(v === 'none' ? '' : v)}
                >
            ...
            {/* Recestation Charges */}
            <DetailSection
                title="Recestation Charges"
                count={recesElements.length}
                color="indigo"
            >
                    ...
                    <div key={rec.id} className="rounded-md border p-2 text-sm">
                        ...
                        <div className="mt-1 flex items-center justify-between text-xs text-gray-500">
                            ...
                            <span>
                                ...
                                {formatMoney(rec.orignal_amount)}
                            </span>
                        </div>
                    </div>
                ))}
            ...
            <DetailSection
                ...
                count={so.expense_vouchers?.length ?? 0}
                color="purple"
            >
            ...
            <div className="space-y-2">
                ...
                {so.treatment_record ? (
                    <div className="rounded-md border p-2 text-sm">
                        <p>
                            Diagnosis:{' '}
                            {so.treatment_record.diagnosis_text ?? '-'}
                        </p>
                        <p>Outcome: {so.treatment_record.outcome ?? '-'}</p>
                        <p>
                            Treated At:{' '}
                            {formatDate(so.treatment_record.treated_at)}
                        </p>
                        <p>
                            Finalized:{' '}
                            {so.treatment_record.is_finalized ? 'Yes' : 'No'}
                        </p>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody>
                                ...
                                {serviceOrders.data.map((serviceOrder) => {
                                    const income =
                                        serviceOrder.income_total ?? 0;
                                    const expense =
                                        serviceOrder.expense_total ?? 0;
                                    const net = income - expense;
                                    const iconUrl = healthIconUrl(
                                        serviceOrder.service?.icon,
                                    );
