        ...
        try {
            for (const n of orderedKeys) {
                if (cancelRef.current) {
                    ...
                    for (const m of orderedKeys.filter(
                        (k) => !statuses[k] || statuses[k] === 'idle',
                    ))
                        setStatus(m, 'skipped');
                    break;
                }
                ...
                <div className="flex w-full items-center justify-center opacity-100 transition-opacity duration-750 lg:grow starting:opacity-0">
                    <main className="flex w-full max-w-xl flex-col-reverse lg:max-w-4xl lg:flex-row">
                        <div className="flex-1 rounded-br-lg rounded-bl-lg bg-white p-6 pb-12 text-[13px] leading-[20px] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] lg:rounded-tl-lg lg:rounded-br-none lg:p-20 dark:bg-[#161615] dark:text-[#EDEDEC] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d]">
                            ...
                            <ul className="mb-4 flex h-12 flex-col lg:mb-6">
                                {orderedKeys.map((n) => {
                                    const s = statuses[n] || 'idle';
                                    const isActive =
                                        currentStep === n && isRunning;
                                    const step = stepsObj[n];
                                    return (
                                        <li
                                            key={n}
                                            className={clsx(
                                                'relative flex items-center gap-4 py-2 before:absolute before:top-1/2 before:bottom-0 before:left-[0.4rem] before:border-l before:border-[#e3e3e0] dark:before:border-[#3E3E3A]',
                                                {
                                                    flex: isActive,
                                                    hidden: !isActive,
                                                },
