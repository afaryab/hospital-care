...
import { counterExpense, counterExpenseNewVoucher, counterExpenseVouchersList, counterView, home, myCounterList } from '@/routes';
...
export default function NewVoucher({ categories, openCounter, users }: { categories: ExpenseCategoryOption[], openCounter?: any, users: User[] }) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        },
        {
            ...
            href: openCounter ? counterExpenseVouchersList().url : myCounterList().url, 
        },
        {
            title: 'New Doctor Voucher',
             href: openCounter ? counterExpenseNewVoucher().url : myCounterList().url,
        }
    ];
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Counter ${openCounter?.ct_number} - New Doctor Voucher`} />
                ...
                <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-2 bg-white text-gray-800">
                    <BulletsWrapper bullets={bullets}>
                        <div className="flex h-full w-full flex-row gap-4 overflow-x-auto rounded-xl p-2 bg-white">
                            <NewVoucherTabs openCounter={openCounter} categories={categories} users={users} />
                        </div>
                    </BulletsWrapper>
                </div>
            </div>
...
function StepIndicator({ step, currentStep, label }: { step: number; currentStep: number; label: string }) {
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            <span className={clsx('text-sm font-medium', isActive ? 'text-blue-600' : isCompleted ? 'text-green-600' : 'text-gray-400')}>
                {label}
            </span>
        </div>
    );
...
function NewVoucherTabs({ openCounter, categories, users }: { openCounter?: any; categories: ExpenseCategoryOption[]; users: User[] }) {
    ...
    return (
        <div className="flex w-full flex-col gap-6">
            ...
            <div className="flex items-center gap-6 border-b pb-4">
                <StepIndicator step={1} currentStep={step} label="Select Category" />
                <div className="h-px w-8 bg-gray-300" />
                <StepIndicator step={2} currentStep={step} label="Paid To" />
                <div className="h-px w-8 bg-gray-300" />
                <StepIndicator step={3} currentStep={step} label="Voucher Details" />
            </div>
            ...
            <div className="flex flex-row gap-4">
                ...
                <div
                    className={clsx(
                        ...
                        step === 1 ? 'w-full' : 'w-1/4 cursor-pointer hover:bg-gray-50',
                    )}
                ...
                {step >= 2 && (
                    <div
                        className={clsx(
                            'flex flex-col gap-4 rounded-lg border p-4 transition-all',
                            step === 2 ? 'flex-1' : 'w-1/4 cursor-pointer hover:bg-gray-50',
                        )}
                ...
                {step >= 3 && (
                    <div className="flex-[2] min-w-0 rounded-lg border p-4">
                        <h2 className="mb-4 text-lg font-semibold">3. Voucher Details</h2>
                        <VoucherDetailsForm
...
function VoucherDetailsForm({
    ...
    const [selectedServiceOrderIds, setSelectedServiceOrderIds] = useState<number[]>([]);
    ...
    const showServiceOrders = !!category?.pay_doc && !category?.pay_others && !category?.pay_users;

    const { errors } = usePage().props as any;

    const fetchServiceOrders = useCallback(async (searchQuery = '', filterDoctorOnly = false, filterClosingId = '') => {
        ...
        try {
            ...
            if (filterClosingId) body.closing_id = parseInt(filterClosingId, 10);
            const response = await fetch('/api/service-orders/completed-unpaid', {
                method: 'POST',
    ...
    const handleSubmit = () => {
        ...
        router.post(
            counterExpenseNewVoucher().url,
            {
                ...
                payed_to: userId,
                service_order_ids: showServiceOrders ? selectedServiceOrderIds : [],
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {errors?.service_order_ids && (
                <div className="rounded-md bg-red-50 p-3 text-sm text-red-700">{errors.service_order_ids}</div>
            )}
                        ...
                        <thead className="sticky top-0 bg-gray-50">
                            <tr className="border-b">
                                <th className="p-2 text-left">
                                    <input
                                        type="checkbox"
                                        checked={serviceOrders.length > 0 && selectedServiceOrderIds.length === serviceOrders.length}
                                        onChange={toggleAll}
                                        className="rounded"
                                    />
                                </th>
                                <th className="p-2 text-left font-medium">SO Number</th>
                            ...
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan={6} className="p-4 text-center text-muted-foreground">
                                        No completed unpaid service orders found.
                                    </td>
                                </tr>
                            ) : (
                                serviceOrders.map((so) => (
                                    <tr
                                        ...
                                        onClick={() => toggleServiceOrder(so.id)}
                                    >
                                        <td className="p-2">
                                            <input
                                                type="checkbox"
                                                checked={selectedServiceOrderIds.includes(so.id)}
                                                onChange={() => toggleServiceOrder(so.id)}
                                                className="rounded"
                                            />
                                        </td>
                                        <td className="p-2 font-medium">{so.so_number}</td>
                                        <td className="p-2">{so.type ?? '-'}</td>
                                        <td className="p-2">{so.patient?.name ?? '-'}</td>
                                        <td className="p-2">{so.doctor?.name ?? '-'}</td>
                                        <td className="p-2">{so.service?.name ?? '-'}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
            ...
            <Button
                ...
                disabled={isSubmitting || !amount || (showServiceOrders && selectedServiceOrderIds.length === 0)}
            >
