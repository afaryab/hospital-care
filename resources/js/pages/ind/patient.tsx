...
interface BedAssignmentData {
    ...
    notes?: string;
    bed?: { id: number; bed_number: string; room?: { id: number; name: string; ward?: { id: number; name: string } } };
...
interface PreviousVisit {
    ...
    status: string;
    created_at: string;
    treatment_record?: { chief_complaint?: string; diagnosis_text?: string; is_finalized: boolean } | null;
}
...
const EXAM_SYSTEMS = ['General', 'Cardiovascular', 'Respiratory', 'Abdomen', 'CNS', 'Musculoskeletal', 'Other'];
const IND_SPECIFIC_FIELDS = ['Admission Type', 'Surgeon', 'Anaesthetist', 'Procedure', 'Blood Group', 'Allergies'];

// ─── Main Component ───────────────────────────────────────────────────────────

export default function IndPatient() {
    const { serviceOrder, bedAssignment, availableBeds, previousVisits } = usePage<IndPatientProps>().props;
    ...
    const [examFindings, setExamFindings] = useState<Record<string, string>>(existingRecord?.examination_findings ?? {});
    const [indSpecificData, setIndSpecificData] = useState<Record<string, string>>(existingRecord?.department_specific_data ?? {});
    const [prescriptions, setPrescriptions] = useState<Prescription[]>(
        existingRecord?.prescriptions?.length ? existingRecord.prescriptions : [blankPrescription()],
    );
    ...
    const [currentAssignment, setCurrentAssignment] = useState<BedAssignmentData | null>(bedAssignment);
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: patient?.name ?? 'Patient', href: indPatient({ id: serviceOrder.id }).url },
    ];
    ...
    const buildPayload = useCallback(
        ...
        [chiefComplaint, hpi, examFindings, diagnosisCode, diagnosisText, treatmentPlan, prescriptions, followUpDate, outcome, referralTo, indSpecificData, vitals],
    );

    const save = useCallback(async (finalize = false) => {
        finalize ? setFinalizing(true) : setSaving(true);
        try {
            const res = await fetch(apiIndSaveTreatment({ serviceOrder: serviceOrder.id }).url, {
                method: 'POST',
                headers: {
    ...
    const assignBed = async () => {
        ...
        setAssigning(true);
        try {
            const res = await fetch(apiIndAssignBed({ serviceOrder: serviceOrder.id }).url, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Accept: 'application/json',
    ...
    const discharge = async () => {
        ...
        setDischarging(true);
        try {
            const res = await fetch(apiIndDischarge({ serviceOrder: serviceOrder.id }).url, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Accept: 'application/json',
            ...
            const json = await res.json();
            if (!res.ok) { toast.error(json.message ?? 'Failed to discharge'); return; }
                    ...
                    <div className="mx-auto max-w-5xl px-4 py-3 md:px-6">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                            <div className="flex items-center gap-3">
                                <a href={indDashboard().url} className="flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-50">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">
                                        {patient?.ps_number} &bull; {ageDisplay(patient)} &bull; {genderLabel(patient?.gender)}
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                {!isFinalized && (
                                    <>
                                        <Button size="sm" variant="outline" disabled={saving} onClick={() => save(false)} className="gap-1.5 text-xs">
                                            <Save className="h-3.5 w-3.5" /> {saving ? 'Saving…' : 'Save Draft'}
                                        </Button>
                                        <Button size="sm" disabled={finalizing} onClick={() => save(true)} className="gap-1.5 bg-indigo-600 text-xs text-white hover:bg-indigo-700">
                                            <CheckCircle className="h-3.5 w-3.5" /> {finalizing ? 'Finalizing…' : 'Finalize'}
                                        </Button>
                                    </>
                                )}
                            </div>
                ...
                <div className="mx-auto max-w-5xl space-y-4 px-4 py-4 pb-10 md:px-6">
                    ...
                    {isFinalized && (
                        <div className="flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3">
                            ...
                            <p className="text-sm font-medium text-emerald-800">This record has been finalized and is read-only.</p>
                        </div>
                    )}
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
                                <h3 className="flex items-center gap-2 text-sm font-semibold text-slate-900">
                                    <History className="h-4 w-4 text-slate-500" /> Previous Indoor Admissions
                                </h3>
                            ...
                            {previousVisits.length === 0 ? (
                                <p className="px-4 py-6 text-center text-sm text-slate-500">No previous IND admissions.</p>
                            ) : (
                                    ...
                                    {previousVisits.map((v) => (
                                        <div key={v.id} className="flex items-start justify-between px-4 py-3">
                                            <div>
                                                ...
                                                {v.treatment_record?.diagnosis_text && <p className="mt-1 text-xs text-slate-600">{v.treatment_record.diagnosis_text}</p>}
                                                {v.treatment_record?.chief_complaint && <p className="mt-0.5 text-xs italic text-slate-400">CC: {v.treatment_record.chief_complaint}</p>}
                                            </div>
                                            <span className={clsx('rounded-full px-2 py-0.5 text-xs font-semibold', statusColor(v.status))}>{v.status}</span>
                                        </div>
                                    ))}
                                </div>
                            )}
                    ...
                    <div className="grid grid-cols-2 gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-4 md:grid-cols-6">
                        ...
                        <InfoCell label="Token" value={serviceOrder.token_short ? `#${serviceOrder.token_short}` : serviceOrder.so_short} />
                        <InfoCell label="Doctor" value={serviceOrder.doctor?.name ?? '—'} />
                        <InfoCell label="Admitted" value={currentAssignment ? formatDate(currentAssignment.admitted_at) : '—'} />
                        <div className="col-span-2">
                            <p className="text-xs text-slate-500">Bed Assignment</p>
                            {currentAssignment ? (
                                <div className="flex items-center gap-2">
                                    <span className="mt-0.5 inline-flex items-center gap-1 rounded-lg bg-indigo-50 px-2 py-1 text-xs font-semibold text-indigo-800 ring-1 ring-indigo-200">
                                        ...
                                        {currentAssignment.ward?.name} / {currentAssignment.room?.name} / Bed {currentAssignment.bed?.bed_number}
                                    </span>
                                    {!isFinalized && (
                                        <button type="button" disabled={discharging} onClick={discharge}
                                ...
                                <div className="mt-0.5 flex items-center gap-2">
                                    <span className="rounded-lg bg-amber-50 px-2 py-1 text-xs font-medium text-amber-700 ring-1 ring-amber-200">Not Assigned</span>
                                    {!isFinalized && (
                                        <button type="button" onClick={() => setShowBedModal(true)}
                    ...
                    <FormSection icon={<ClipboardList className="h-4 w-4 text-indigo-500" />} title="Admission Details">
                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                            {IND_SPECIFIC_FIELDS.map((field) => (
                                <div key={field}>
                                    <label className="mb-1 block text-xs font-medium text-slate-500">{field}</label>
                                    <input disabled={isFinalized} value={indSpecificData[field] ?? ''}
                                        onChange={(e) => setIndSpecificData((d) => ({ ...d, [field]: e.target.value }))}
                    ...
                    {/* Chief Complaint */}
                    <FormSection icon={<AlertCircle className="h-4 w-4 text-red-500" />} title="Chief Complaint / Presenting Problem">
                        <textarea disabled={isFinalized} value={chiefComplaint} onChange={(e) => setChiefComplaint(e.target.value)}
                            rows={2} placeholder="Primary reason for admission…" className={textareaClass(isFinalized)} />
                    </FormSection>
                    ...
                    <FormSection icon={<FileText className="h-4 w-4 text-slate-500" />} title="History of Present Illness">
                        <textarea disabled={isFinalized} value={hpi} onChange={(e) => setHpi(e.target.value)}
                            rows={4} placeholder="Detailed history, onset, progression, relevant past medical/surgical history…"
                    ...
                    <FormSection icon={<Stethoscope className="h-4 w-4 text-indigo-600" />} title="Examination Findings">
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                            {EXAM_SYSTEMS.map((sys) => (
                                <div key={sys}>
                                    <label className="mb-1 block text-xs font-medium text-slate-500">{sys}</label>
                                    <input disabled={isFinalized} value={examFindings[sys] ?? ''}
                                        onChange={(e) => setExamFindings((f) => ({ ...f, [sys]: e.target.value }))}
                                        placeholder={`${sys} findings…`} className={inputClass(isFinalized)} />
                                </div>
                            ))}
                        </div>
                    </FormSection>
                    ...
                    {/* Prescription */}
                    <FormSection icon={<FileText className="h-4 w-4 text-emerald-600" />} title="Medication Orders / Prescription">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {prescriptions.map((row, idx) => (
                                        <tr key={idx} className="bg-white hover:bg-slate-50">
                                            ...
                                            <td className="px-2 py-1.5"><input disabled={isFinalized} value={row.route} onChange={(e) => updatePrescription(idx, 'route', e.target.value)} placeholder="Oral/IV/IM" className={tableInputClass(isFinalized)} /></td>
                                            <td className="px-2 py-1.5"><input disabled={isFinalized} value={row.instructions} onChange={(e) => updatePrescription(idx, 'instructions', e.target.value)} placeholder="After meals…" className={tableInputClass(isFinalized)} /></td>
                                            <td className="px-2 py-1.5">
                                                {!isFinalized && prescriptions.length > 1 && (
                                                    <button type="button" onClick={() => removePrescription(idx)} className="text-slate-400 hover:text-red-500"><Trash2 className="h-4 w-4" /></button>
                                                )}
                                            </td>
                                        </tr>
                                    ))}
                    ...
                    {/* Treatment Plan */}
                    <FormSection icon={<ClipboardList className="h-4 w-4 text-orange-500" />} title="Treatment Plan / Management Notes">
                        <textarea disabled={isFinalized} value={treatmentPlan} onChange={(e) => setTreatmentPlan(e.target.value)}
                            rows={4} placeholder="Surgical plan, investigations ordered, consults, nursing orders, diet, activity restrictions…"
                            className={textareaClass(isFinalized)} />
                    </FormSection>

                    {/* Outcome / Discharge Planning */}
                    <FormSection icon={<Calendar className="h-4 w-4 text-blue-500" />} title="Discharge Planning & Outcome">
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                            <div>
                                <label className="mb-1 block text-xs font-medium text-slate-500">Follow-up Date</label>
                                <input disabled={isFinalized} type="date" value={followUpDate} onChange={(e) => setFollowUpDate(e.target.value)} className={inputClass(isFinalized)} />
                            </div>
                            <div>
                                <label className="mb-1 block text-xs font-medium text-slate-500">Outcome</label>
                                <select disabled={isFinalized} value={outcome} onChange={(e) => setOutcome(e.target.value)} className={inputClass(isFinalized)}>
                            ...
                            <div>
                                ...
                                <input disabled={isFinalized} value={referralTo} onChange={(e) => setReferralTo(e.target.value)} placeholder="Department / Hospital" className={inputClass(isFinalized)} />
                            </div>
                        </div>
                    </FormSection>
                    ...
                    {!isFinalized && (
                        <div className="flex items-center justify-end gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                            ...
                            <Button disabled={finalizing} onClick={() => save(true)} className="gap-1.5 bg-indigo-600 text-white hover:bg-indigo-700">
                                <CheckCircle className="h-4 w-4" /> {finalizing ? 'Finalizing…' : 'Finalize & Discharge Plan'}
                            </Button>
                        </div>
                    )}
                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                    <div className="w-full max-w-sm rounded-2xl bg-white shadow-2xl">
                        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                            ...
                            <button type="button" onClick={() => { setShowBedModal(false); setSelectedBedId(''); }}
                        ...
                        <div className="p-5">
                            <div className="mb-3 rounded-xl bg-slate-50 px-4 py-3">
                                ...
                                <p className="text-xs text-slate-500">{serviceOrder.so_number}</p>
                            </div>
                            ...
                            {availableBedOptions.length === 0 && (
                                <p className="mt-2 text-xs text-amber-600">No available beds right now.</p>
                            )}
                        </div>
...
function FormSection({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) {
    ...
    return (
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <button type="button" onClick={() => setCollapsed((c) => !c)} className="flex w-full items-center justify-between px-4 py-3 md:px-5">
                <div className="flex items-center gap-2">{icon}<span className="text-sm font-semibold text-slate-900">{title}</span></div>
                {collapsed ? <ChevronDown className="h-4 w-4 text-slate-400" /> : <ChevronUp className="h-4 w-4 text-slate-400" />}
            </button>
            {!collapsed && <><div className="h-px bg-slate-100" /><div className="p-4 md:p-5">{children}</div></>}
        </div>
    );
}
...
function VitalInput({ label, placeholder, icon, value, onChange, disabled }: {
    label: string; placeholder: string; icon: React.ReactNode; value: string; onChange: (v: string) => void; disabled: boolean;
}) {
    return (
        <div>
            <label className="mb-1 flex items-center gap-1 text-xs font-medium text-slate-500">{icon} {label}</label>
            <input disabled={disabled} type="number" step="any" value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className={inputClass(disabled)} />
        </div>
    );
}
...
function textareaClass(disabled: boolean) {
    return clsx(
        'w-full rounded-xl border px-3 py-2.5 text-sm text-slate-800 placeholder:text-slate-400 resize-y focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-400',
        disabled ? 'border-slate-100 bg-slate-50 text-slate-600 cursor-not-allowed' : 'border-slate-200 bg-white hover:border-slate-300',
    );
}

function inputClass(disabled: boolean) {
    return clsx(
        'w-full rounded-xl border px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-400',
        disabled ? 'border-slate-100 bg-slate-50 text-slate-600 cursor-not-allowed' : 'border-slate-200 bg-white hover:border-slate-300',
    );
}
...
function tableInputClass(disabled: boolean) {
    return clsx(
        'w-full rounded-lg border px-2 py-1.5 text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-indigo-200 focus:border-indigo-300',
        disabled ? 'border-transparent bg-transparent text-slate-600 cursor-not-allowed' : 'border-slate-200 bg-white hover:border-slate-300',
    );
}
