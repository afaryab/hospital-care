...
export default function IndDashboard() {
    ...
    const availableBedOptions = wards.flatMap((w) =>
        w.rooms.flatMap((r) =>
            r.beds
                ...
                .map((b) => ({
                    id: b.id,
                    label: `${w.name} → ${r.name} → Bed ${b.bed_number}`,
                })),
        ),
    );
    ...
    const openOrder = (order: IndServiceOrder) =>
        router.visit(indPatient({ id: order.id }).url);
    ...
    const assignBed = async () => {
        ...
        try {
            const res = await fetch(
                apiIndAssignBed({ serviceOrder: assignModal.order.id }).url,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
...
function WardCard({
    ...
    ward: WardData;
    onOpenPatient: (order: IndServiceOrder) => void;
}) {
    const [collapsed, setCollapsed] = useState(false);
    const totalBeds = ward.rooms
        .flatMap((r) => r.beds)
        .filter((b) => b.is_active).length;
    const occupiedBeds = ward.rooms
        .flatMap((r) => r.beds)
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    <div className="text-left">
                        ...
                        <p className="text-xs opacity-70">
                            {ward.floor && `${ward.floor} Floor`}
                            {ward.building && ` · ${ward.building}`} &bull;{' '}
        ...
        <div className="px-4 py-3">
            <div className="mb-2 flex items-center gap-2">
                ...
                <p className="text-xs font-semibold opacity-80">
                    {room.name}
                    {room.room_number ? ` (#${room.room_number})` : ''} &bull;
                    Cap. {room.capacity}
                </p>
            </div>
...
function BedBlock({
    ...
    return (
        <div
            ...
            title={
                isOccupied
                    ? `Open ${assignment?.patient?.name}'s file`
                    : bed.status
            }
        >
