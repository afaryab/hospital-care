...
export default function FilterAndSelectServiceOrder({
	...
	const [selected, setSelected] = useState<ServiceOrderSearchItem | null>(null);
					...
					{allQuickResults.map((item) => (
						...
						>
							...
							<div className="text-xs text-muted-foreground">
								{item.patient?.name ?? 'N/A'} | {item.service?.name ?? 'N/A'}
							</div>
						</button>
					))}
				</div>
			...
			<Dialog open={isAdvancedOpen} onOpenChange={setIsAdvancedOpen}>
				<DialogContent className="max-w-5xl">
					<DialogHeader>
						...
						<DialogDescription>Filter service orders and select one from results.</DialogDescription>
					</DialogHeader>
					...
					<div className="grid grid-cols-1 gap-3 md:grid-cols-2">
						<div className="space-y-1">
							...
							<Input
								...
								onChange={(event) => setFilters((prev) => ({ ...prev, so_number: event.target.value }))}
							/>
						</div>
						<div className="space-y-1">
							<Label>Type</Label>
						...
						<div className="space-y-1">
							<FilterAndSelectPatient
								...
								onValueChange={(value) => setFilters((prev) => ({ ...prev, patient_id: value }))}
								label="Patient"
						...
						<div className="space-y-1">
							<FilterAndSelectUser
								...
								onValueChange={(value) => setFilters((prev) => ({ ...prev, doctor_id: value }))}
								doctorOnly
								label="Doctor"
