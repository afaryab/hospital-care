...
import { Activity, AlertCircle, ChevronRight, Clock, Search, UserCheck, Users } from 'lucide-react';
...
export default function DeptQueueDashboard({
    deptName, accentClass, hasAccess, orders, stats, noAccessMessage, searchUrl, patientUrl, flashError,
}: Props) {
    ...
    return (
        <div className="flex flex-1 flex-col gap-4 p-4">
            ...
            <div className="flex items-center justify-between">
                <h1 className={clsx('text-2xl font-bold', accentClass)}>{deptName} Portal</h1>
            ...
            <div className="grid grid-cols-4 gap-3">
                {[
                    { label: 'Waiting', value: stats.open, icon: <Clock className="h-4 w-4 text-amber-500" />, color: 'bg-amber-50 text-amber-700' },
                    { label: 'In Progress', value: stats.in_progress, icon: <Activity className="h-4 w-4 text-blue-500" />, color: 'bg-blue-50 text-blue-700' },
                    { label: 'Treated', value: stats.treated, icon: <UserCheck className="h-4 w-4 text-emerald-500" />, color: 'bg-emerald-50 text-emerald-700' },
                    { label: 'Total', value: stats.total, icon: <Users className="h-4 w-4 text-slate-500" />, color: 'bg-slate-100 text-slate-700' },
                ].map((s) => (
                    <div key={s.label} className={clsx('flex items-center gap-3 rounded-xl px-4 py-3', s.color)}>
                        {s.icon}
                        <div>
            ...
            {/* Search */}
            <form ref={formRef} action={searchUrl} method="GET" className="flex gap-2">
            ...
            {orders.length === 0 ? (
                <div className="flex flex-1 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 py-16 text-center">
                    ...
                    <p className="text-sm font-medium text-slate-400">No orders assigned to you today</p>
                </div>
            ) : (
                            ...
                            {orders.map((order) => (
                                ...
                                >
                                    <td className="px-4 py-3 font-medium text-slate-800">
                                        ...
                                        <div className="text-xs text-slate-400">{order.patient?.ps_number}</div>
                                    </td>
                                    <td className="px-4 py-3 text-slate-600">{order.service?.name ?? '—'}</td>
                                    <td className="px-4 py-3 text-slate-500">
                                        {order.patient ? formatPatientAge(order.patient) : '—'}
                                        {order.patient?.gender && <span className="ml-1 text-xs text-slate-400">/ {order.patient.gender.toUpperCase()}</span>}
                                    </td>
                                    <td className="px-4 py-3">{statusBadge(order.status)}</td>
                                    <td className="px-4 py-3 font-mono text-xs text-slate-400">{order.so_number}</td>
