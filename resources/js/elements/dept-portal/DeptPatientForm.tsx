...
interface VitalSign {
    temperature?: number | string; bp_systolic?: number | string; bp_diastolic?: number | string;
    pulse_rate?: number | string; respiratory_rate?: number | string; oxygen_saturation?: number | string;
    weight?: number | string; height?: number | string;
}
...
export interface DeptPatientFormProps {
    ...
    serviceOrder: {
        id: number; so_number: string; so_short?: string; status: string; token?: string | number; created_at?: string;
        patient?: Patient | null; service?: { id: number; name: string } | null;
        doctor?: { id: number; name: string } | null; treatment_record?: TreatmentRecord | null;
    };
...
function textareaClass(disabled: boolean) {
    return clsx(
        'w-full rounded-xl border px-3 py-2.5 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-100 focus:border-slate-400 resize-y',
...
function inputClass(disabled: boolean) {
    return clsx(
        'w-full rounded-xl border px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-100 focus:border-slate-400',
        disabled ? 'border-slate-100 bg-slate-50 text-slate-600 cursor-not-allowed' : 'border-slate-200 bg-white hover:border-slate-300',
    );
}
...
function tableInputClass(disabled: boolean) {
    return clsx(
        'w-full rounded-lg border px-2 py-1.5 text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-slate-200 focus:border-slate-300',
...
export default function DeptPatientForm({
    ...
    const [vitals, setVitals] = useState({
        ...
        bp_diastolic: lastVital.bp_diastolic ?? '', pulse_rate: lastVital.pulse_rate ?? '',
        respiratory_rate: lastVital.respiratory_rate ?? '', oxygen_saturation: lastVital.oxygen_saturation ?? '',
        weight: lastVital.weight ?? '', height: lastVital.height ?? '',
    });
    ...
    const buildPayload = useCallback((finalize = false) => ({
        chief_complaint: chiefComplaint, history_of_present_illness: hpi,
        examination_findings: showExamFindings ? examFindings : undefined,
        diagnosis_code: diagnosisCode, icd10_code_id: icd10CodeId, diagnosis_text: diagnosisText,
        treatment_plan: treatmentPlan,
        prescriptions: showPrescriptions ? prescriptions.filter((p) => p.drug_name.trim()) : [],
    ...
    const updateRx = (idx: number, field: keyof Prescription, value: string) => {
                ...
                <div className="mx-auto max-w-5xl px-4 py-3 md:px-6">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                            ...
                            <div className={clsx('flex h-10 w-10 items-center justify-center rounded-full text-base font-bold text-white shadow-sm', accentColor)}>
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap items-center gap-x-2 text-xs text-slate-500">
                                    ...
                                    {patient && <><span>&bull;</span><span>{formatPatientAge(patient)}</span></>}
                                    {patient?.gender && <><span>&bull;</span><span>{patient.gender.toUpperCase()}</span></>}
                                    {patient?.contact && <><span>&bull;</span><span>{patient.contact}</span></>}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            >
                                <History className="h-3.5 w-3.5" /> History ({previousVisits.length})
                            </button>

                            {!isFinalized && serviceOrder.status.toLowerCase() === 'open' && (
                                <button
                                    type="button"
                                    disabled={calling}
                                    onClick={callPatient}
                                    className={clsx('rounded-lg px-3 py-1.5 text-xs font-semibold text-white transition-colors disabled:opacity-50', accentColor, 'hover:opacity-90')}
                                >
                            ...
                            {!isFinalized && (
                                <>
                                    ...
                                    <button
                                        ...
                                        disabled={finalizing}
                                        onClick={() => save(true)}
                                        className={clsx('flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50', accentColor, 'hover:opacity-90')}
                                    >
            ...
            <div className="mx-auto max-w-5xl px-4 pb-10 pt-4 md:px-6">
                ...
                {isFinalized && (
                    <div className="mb-4 flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3">
                        ...
                        <p className="text-sm font-medium text-emerald-800">This treatment record has been finalized and is read-only.</p>
                    </div>
                )}
                    ...
                    <div className="mb-4 rounded-2xl border border-slate-200 bg-white shadow-sm">
                        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
                            <h3 className="flex items-center gap-2 text-sm font-semibold text-slate-900">
                                <History className="h-4 w-4 text-slate-500" /> Previous {deptName} Visits
                            </h3>
                        ...
                        {previousVisits.length === 0 ? (
                            <p className="px-4 py-6 text-center text-sm text-slate-500">No previous visits.</p>
                        ) : (
                                    ...
                                    <div key={v.id} className="flex items-start justify-between px-4 py-3">
                                        <div>
                                            ...
                                            {v.treatment_record?.chief_complaint && (
                                                <p className="mt-0.5 text-xs italic text-slate-400">CC: {v.treatment_record.chief_complaint}</p>
                                            )}
                                        </div>
                ...
                <div className="space-y-4">
                    ...
                    <FormSection icon={<AlertCircle className="h-4 w-4 text-red-500" />} title={chiefComplaintLabel}>
                        <textarea
                            disabled={isFinalized} value={chiefComplaint} rows={2}
                    ...
                    <FormSection icon={<FileText className="h-4 w-4 text-slate-500" />} title="History of Present Illness">
                        <textarea
                            disabled={isFinalized} value={hpi} rows={3}
                    ...
                    {showVitals && (
                        <FormSection icon={<Heart className="h-4 w-4 text-rose-500" />} title="Vital Signs">
                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                                {[
                                    { key: 'temperature', label: 'Temp (°C)', placeholder: '37.0', icon: <Thermometer className="h-3.5 w-3.5" /> },
                                    { key: 'bp_systolic', label: 'BP Systolic', placeholder: '120', icon: <Heart className="h-3.5 w-3.5" /> },
                                    { key: 'bp_diastolic', label: 'BP Diastolic', placeholder: '80', icon: <Heart className="h-3.5 w-3.5" /> },
                                    { key: 'pulse_rate', label: 'Pulse (bpm)', placeholder: '72', icon: <Activity className="h-3.5 w-3.5" /> },
                                    { key: 'respiratory_rate', label: 'Resp. Rate', placeholder: '16', icon: <Wind className="h-3.5 w-3.5" /> },
                                    { key: 'oxygen_saturation', label: 'SpO2 (%)', placeholder: '98', icon: <Activity className="h-3.5 w-3.5" /> },
                                    { key: 'weight', label: 'Weight (kg)', placeholder: '70', icon: <User className="h-3.5 w-3.5" /> },
                                    { key: 'height', label: 'Height (cm)', placeholder: '170', icon: <User className="h-3.5 w-3.5" /> },
                                ].map(({ key, label, placeholder, icon: vIcon }) => (
                                    <div key={key}>
                                        <label className="mb-1 flex items-center gap-1 text-xs font-medium text-slate-500">{vIcon} {label}</label>
                                        <input
                                            disabled={isFinalized} type="number" step="any"
                                            value={String(vitals[key as keyof typeof vitals])}
                            ...
                            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                {examSystems.map((sys) => (
                                    <div key={sys}>
                                        ...
                                        <input
                                            disabled={isFinalized} value={examFindings[sys] ?? ''}
                    ...
                    <FormSection icon={<FileText className="h-4 w-4 text-indigo-500" />} title="Diagnosis">
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                            <div>
                                ...
                                <Icd10Picker
                                    value={diagnosisCode} disabled={isFinalized}
                                    placeholder="Search code or diagnosis…"
                                    className={inputClass(isFinalized)}
                                    onSelect={(code, description) => { setDiagnosisCode(code); setDiagnosisText(description); }}
                                />
                            </div>
                            <div className="sm:col-span-2">
                                <label className="mb-1 block text-xs font-medium text-slate-500">Diagnosis</label>
                                <input
                                    disabled={isFinalized} value={diagnosisText}
                    ...
                    <FormSection icon={<FileText className="h-4 w-4 text-violet-500" />} title={treatmentPlanLabel}>
                        <textarea
                            disabled={isFinalized} value={treatmentPlan} rows={6}
                    ...
                    {showPrescriptions && (
                        <FormSection icon={<FileText className="h-4 w-4 text-emerald-600" />} title="Prescription">
                                            ...
                                            <tr key={idx} className="bg-white hover:bg-slate-50">
                                                <td className="px-2 py-1.5">
                                                    {isFinalized ? (
                                                        ...
                                                        <DrugPicker
                                                            value={row.drug_name}
                                                            onChange={(name) => updateRx(idx, 'drug_name', name)}
                                                            onSelect={(drug) => {
                                                ...
                                                <td className="px-2 py-1.5"><input disabled={isFinalized} value={row.dose ?? ''} onChange={(e) => updateRx(idx, 'dose', e.target.value)} placeholder="500mg" className={tableInputClass(isFinalized)} /></td>
                                                <td className="px-2 py-1.5"><input disabled={isFinalized} value={row.frequency ?? ''} onChange={(e) => updateRx(idx, 'frequency', e.target.value)} placeholder="TDS" className={tableInputClass(isFinalized)} /></td>
                                                <td className="px-2 py-1.5"><input disabled={isFinalized} value={row.duration ?? ''} onChange={(e) => updateRx(idx, 'duration', e.target.value)} placeholder="5 days" className={tableInputClass(isFinalized)} /></td>
                                                <td className="px-2 py-1.5"><input disabled={isFinalized} value={row.route ?? ''} onChange={(e) => updateRx(idx, 'route', e.target.value)} placeholder="Oral" className={tableInputClass(isFinalized)} /></td>
                                                <td className="px-2 py-1.5"><input disabled={isFinalized} value={row.instructions ?? ''} onChange={(e) => updateRx(idx, 'instructions', e.target.value)} placeholder="After meals" className={tableInputClass(isFinalized)} /></td>
                                                <td className="px-2 py-1.5">
                                                    {!isFinalized && prescriptions.length > 1 && (
                                                        <button type="button" onClick={() => setPrescriptions((p) => p.filter((_, i) => i !== idx))} className="text-slate-400 transition-colors hover:text-red-500">
                            ...
                            {!isFinalized && (
                                <button type="button" onClick={() => setPrescriptions((p) => [...p, blankRx()])}
                    ...
                    {!isFinalized && (
                        <div className="flex items-center justify-end gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                            ...
                            <button type="button" disabled={finalizing} onClick={() => save(true)}
                                className={clsx('flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-semibold text-white disabled:opacity-50', accentColor, 'hover:opacity-90')}>
