...
export default function FilterAndSelectTransaction({
	...
	const [selected, setSelected] = useState<TransactionSearchItem | null>(null);
					...
					{allQuickResults.map((item) => (
						...
						>
							...
							<div className="text-xs text-muted-foreground">
								{item.income_or_expense ?? 'N/A'} | {item.type ?? 'N/A'} | {item.amount ?? 0}
							</div>
						</button>
					))}
				</div>
			...
			{selected ? (
				<div className="flex items-center justify-between rounded-md border bg-muted/20 p-2 text-sm">
					<div>
						...
						<div className="text-xs text-muted-foreground">{selected.patient?.name ?? 'No patient linked'}</div>
					</div>
			...
			<Dialog open={isAdvancedOpen} onOpenChange={setIsAdvancedOpen}>
				<DialogContent className="max-w-5xl">
					<DialogHeader>
						...
						<DialogDescription>Filter transactions and select one from results.</DialogDescription>
					</DialogHeader>
					...
					<div className="grid grid-cols-1 gap-3 md:grid-cols-2">
						<div className="space-y-1">
							...
							<Input
								...
								onChange={(event) => setFilters((prev) => ({ ...prev, tr_number: event.target.value }))}
							/>
						</div>
						<div className="space-y-1">
							<Label>Type</Label>
							<Input value={filters.type} onChange={(event) => setFilters((prev) => ({ ...prev, type: event.target.value }))} />
						</div>
						<div className="space-y-1">
							<Label>Income/Expense</Label>
							<Input
								value={filters.income_or_expense}
								onChange={(event) => setFilters((prev) => ({ ...prev, income_or_expense: event.target.value.toUpperCase() }))}
