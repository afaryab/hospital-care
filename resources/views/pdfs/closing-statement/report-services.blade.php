...
<head>
    ...
    <title>Services Report - {{ $closing['ct_number'] }}</title>
...
<body>
    ...
    {{-- Provider Summary Table at Top --}}
    @if(count($service_groups) > 0)
    @php
        // Collect all providers across services with totals
        $providerSummary = [];
    ...
    {{-- Services grouped by Service → Provider --}}
    @if(count($service_groups) > 0)
        @foreach($service_groups as $group)
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $item['service_recestation'] ?? '-' }}</td>
                            <td>
                                @php
        ...
        @endforeach
    @else
        <div class="empty-state">No service transactions found for this counter.</div>
    @endif
    ...
    @if(count($expenses_by_doctor) > 0)
        ...
        <table>
            ...
            <tfoot>
                <tr class="total-row">
                    <td colspan="4" class="text-right">Total Expenses Paid</td>
